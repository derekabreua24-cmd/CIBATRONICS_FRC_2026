...
| Use `resetFieldForAuto()` to populate field with Fuel | Called once when `m_fieldInitialized` is false in `MapleSimHandler` |
...
| Launch projectile with `RebuiltFuelOnFly` and `addGamePieceProjectile()` | `SimLaunchNoteCommand` builds `RebuiltFuelOnFly` and calls `SimulatedArena.getInstance().addGamePieceProjectile(fuel)` |
| Configure projectile to become field piece on touchdown | `fuel.enableBecomesGamePieceOnFieldAfterTouchGround()` |

## Version

- **Vendordep:** `vendordeps/maple-sim.json` → `maplesim-java` **0.4.0-beta** (2026 Rebuilt support).
- **Install from:** https://shenzhen-robotics-alliance.github.io/maple-sim/vendordep/maple-sim.json (per official installing page).

## Code locations

- **Arena, chassis, intake, FUEL logging, goals, vision injection:** `frc.robot.simulation.MapleSimHandler`
- **Kinematic chassis (differential, no wheel physics):** `frc.robot.simulation.KinematicChassisSim`
- **Launch FUEL projectile (sim only):** `frc.robot.simulation.SimLaunchNoteCommand`
- **Sim initial pose / hub poses:** `frc.robot.constants.MapleSimConstants`

...
| **Sim collision** | In simulation, `DriveSubsystem.simulationPeriodic()` clamps the drivetrain sim pose to the field boundary and to obstacle boxes (`kSimObstacles`). On collision, velocity is zeroed via `setState()` so the robot stops. Logged pose stays at the boundary so AdvantageScope shows correct collision. |
| **Sim game pieces** | `GamePieceSimulation` (sim only) models balls with gravity. Balls spawn at fixed positions; intake picks them up when the intake motor is running and a ball is in the intake zone; shooter launches balls when at speed (and carried &gt; 0). Logged as `Sim/Balls` (Pose2d[]) and `Sim/CarriedCount` for AdvantageScope 3D Field / game piece views. |
